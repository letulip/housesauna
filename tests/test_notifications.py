...
def test_send_telegram_success(mock_bot_class, caplog):
    """sendMessage() вызывается правильно."""
    mock_bot = MagicMock()
    mock_bot_class.return_value = mock_bot

    with caplog.at_level("INFO", logger="housesauna.utility"):
    ...
    assert "[Telegram] Отправлено: Test message" in caplog.text
...
def test_send_telegram_failure(mock_bot_class, caplog):
    """Ошибка логируется."""
    with caplog.at_level("WARNING", logger="housesauna.utility"):
